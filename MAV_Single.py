...
import numpy as np
import matplotlib.pyplot as plt
from pyomo.environ import sqrt, tanh
...
class MAV():
    ...
    def __init__(self,
        ...
        self.m.x_max         = Param(initialize = 5000e3)
        self.m.y_max         = Param(initialize = 5000e3)
        self.m.z_max         = Param(initialize = 200e3)
        ...
        self.m.mpdot_min     = Param(initialize = 1e-6)
        self.m.mpdot_max     = Param(initialize = 7.5)

        self.m.massdot_min   = Param(initialize = -1e-6)
        ...
        self.m.mass_scale    = Param(initialize = 200)
        self.m.massdot_scale = Param(initialize = 1)
        ...
        self.m.zdot_1 = Var(self.m.t1, bounds=(self.m.zdot_min / self.m.z_scale / self.m.zdot_scale, None))
        ...
        # Add bounds for body acceleration variables
        self.m.udot_1 = Var(self.m.t1, bounds=(self.m.udot_min /  self.m.u_scale / self.m.udot_scale, self.m.udot_max /  self.m.u_scale / self.m.udot_scale))
        ...
        # quaternion representation state variables
        self.m.q0_1 = Var(self.m.t1, bounds=(-1 / self.m.q0_scale , 1 / self.m.q0_scale))
        self.m.q1_1 = Var(self.m.t1, bounds=(-1 / self.m.q1_scale , 1 / self.m.q1_scale))
        self.m.q2_1 = Var(self.m.t1, bounds=(-1 / self.m.q2_scale , 1 / self.m.q2_scale))
        self.m.q3_1 = Var(self.m.t1, bounds=(-1 / self.m.q3_scale , 1 / self.m.q3_scale))
        ...
        # mass properties state variables
        self.m.mass_1    = Var(self.m.t1, bounds=(self.m.mass_min / self.m.mass_scale, self.m.mass_max / self.m.mass_scale))
        self.m.massdot_1 = Var(self.m.t1, bounds=(self.m.massdot_max/ self.m.mass_scale / self.m.massdot_scale, self.m.massdot_min / self.m.mass_scale / self.m.massdot_scale))
        self.m.dmass_dtau_1 = DerivativeVar(self.m.mass_1, wrt=self.m.t1)
        ...
        # aerodynamic angles
        self.m.alpha_1 = Var(self.m.t1, bounds=(self.m.alpha_min / self.m.alpha_scale, self.m.alpha_max / self.m.alpha_scale), initialize=0.0)
        self.m.beta_1 = Var(self.m.t1, bounds=(self.m.beta_min / self.m.beta_scale, self.m.beta_max / self.m.beta_scale), initialize=0.0)
        
        # discretize problem euler backward finite difference
        # discretizer = TransformationFactory('dae.finite_difference')
        # discretizer.apply_to(self.m, nfe=70, wrt=self.m.t1, scheme='BACKWARD')

        discretizer = TransformationFactory('dae.collocation')
        discretizer.apply_to(self.m, wrt=self.m.t1, nfe=50, ncp=3, scheme='LAGRANGE-RADAU') 
    ...
    def Q_normality(self, m, n,t):
        ...
        return 1 - (((q0**2) + (q1**2) + (q2**2) + (q3**2)) )**2 <= 1e-8
    ...
    def Q_udot(self, m, n, t): 
        ...
        Mach_eff = (((u**2 + v**2 + w**2 + 0.001))**0.5) / a
        Mach = 3.0 * tanh(Mach_eff / 3.0)
        CX = (aero.forces.CX_alpha(Mach, alpha, beta)) + (aero.forces.CX_beta(Mach, alpha, beta))
        AX = 0.5 * atm.rho(z) * (u ** 2)  * param.S * CX
        FX = (((mpdot) * prop.Isp * 9.81) * cos(kap) * cos(eps))
        return (udot) == (((FX / mass)) - ((w)  * (q)) + ((v) * (r)) + ((eom.quaternion.Q13(q0, q1, q2, q3)* atm.gravity(z))))
    ...
    def Q_vdot(self, m, n, t): 
        ...
        CY = aero.forces.CN_beta(Mach, alpha, beta)
        AY = 0.5 * atm.rho(z) * ((v) ** 2) * param.S * CY
        FY = -(((mpdot) * prop.Isp * 9.81) * cos(kap) * sin(eps))
        return (vdot) == ((( (FY) / mass)) - ((u) * (r)) + ((w) * (p)) + ((eom.quaternion.Q23(q0, q1, q2, q3) * atm.gravity(z))))
    ...
    def Q_wdot(self, m, n, t): 
        ...
        CZ = aero.forces.CN_alpha(Mach, alpha, beta)
        AZ = 0.5 * atm.rho(z) * ((w) ** 2) * param.S * CZ
        FZ = -(((mpdot) * prop.Isp * 9.81) * sin(kap))
        return (wdot) == ((( (FZ) / mass)) - ((v) * (p)) + ((u) * (q)) + ((eom.quaternion.Q33(q0, q1, q2, q3) * atm.gravity(z))))
    ...
    def Q_pdot(self, m, n, t): 
        ...
        CL = 10e-7
        AL = 0.5 * atm.rho(z) * ((u) ** 2) * param.S * param.l * CL
        return (pdot) == (((q) * (r) ) * ((param.Iy - param.Iz) / param.Ix))
    ...
    def Q_qdot(self, m, n, t): 
        ...
        CM = aero.moments.CM_alpha(Mach, alpha, beta)
        AM = 0.5 * atm.rho(z) * ((v) ** 2) * param.S * param.l * CM
        return (qdot) == ((((p) * (r)) * ((param.Iz - param.Ix) / param.Iy))- ((MZ ) / param.Iy))
    ...
    def Q_rdot(self, m, n, t): 
        ...
        CN = aero.moments.CM_beta(Mach, alpha, beta)
        AN = 0.5 * atm.rho(z) * ((w) ** 2) * param.S * param.l * CN
        return (rdot) == ((((p) * (q)) * ((param.Ix - param.Iy) / param.Iz)) + ((MY ) / param.Iz))
    ...
    def Q_phi(self, m, n, t): 
        ...
        (tf, x, y, z, xdot, ydot, zdot, \
            ...
            dmass_dtau, \
            dq0_dtau, dq1_dtau, dq2_dtau, dq3_dtau, q0dot, q1dot, q2dot, q3dot) = getPhaseVariables(m, n, t)
        return atan(2 * (q2 * q3 + q0 * q1) / (q0**2 - q1**2 - q2**2 - q3**2)) 
    ...
    def Q_psi(self, m, n, t): 
        ...
        (tf, x, y, z, xdot, ydot, zdot, \
            ...
            dmass_dtau, \
            dq0_dtau, dq1_dtau, dq2_dtau, dq3_dtau, q0dot, q1dot, q2dot, q3dot) = getPhaseVariables(m, n, t)
        return atan((2 * (q1 * q2 + q0 * q3)) / (q0**2 + q1**2 - q2**2 - q3**2))
...
#================Phase 1==============================================
        ...
        # yield m.q1_1[0]          == 0.0001/ m.q1_scale
        # yield m.q2_1[0]          == 0.674287911628145/ m.q2_scale
        # yield m.q3_1[0]          == 0.0001 / m.q1_scale 
        ...
        # yield m.p_1[0]          == 0.01 / m.p_scale
        # yield m.q_1[0]          == 0.01 / m.q_scale
        # yield m.r_1[0]          == 0.01 / m.r_scale

        # yield m.the_1[0]        == (1.48) / m.the_scale 
        # yield m.psi_1[0]       == (0.001745277) / m.psi_scale  
        # yield m.phi_1[0]       == (0.0001) / m.phi_scale  

        yield m.mass_1[0]       == 290 / m.mass_scale
        yield m.mpdot_1[0]       == 0.001 / m.mpdot_scale 

        # yield m.u_1[1]          == 1 / m.u_scale

        yield m.z_1[1]         == 200e3 /  self.m.z_scale
        # yield m.zdot_1[1]         == 1 /  self.m.z_scale / self.m.zdot_scale
