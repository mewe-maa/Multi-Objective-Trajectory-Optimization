...
class MAV():
    ...
    def __init__(self):
        ...
        self.m.mass_min      = Param(initialize = 50)
        self.m.mass_max      = Param(initialize = 290)

        self.m.mpdot_min     = Param(initialize = 0.00001)
        self.m.mpdot_max     = Param(initialize = 7.5)

        self.m.massdot_min   = Param(initialize = -0.00001)
        ...
        self.m.xdot_scale   = Param(initialize = 0.05)
        self.m.ydot_scale   = Param(initialize = 0.05)
        self.m.zdot_scale   = Param(initialize = 0.05)

        self.m.u_scale      = Param(initialize = 600)
        self.m.v_scale      = Param(initialize = 890)
        self.m.w_scale      = Param(initialize = 890)

        self.m.udot_scale   = Param(initialize = 0.05)
        self.m.vdot_scale   = Param(initialize = 0.05)
        self.m.wdot_scale   = Param(initialize = 0.05)
        ...
        # mass properties state variables
        self.m.mass_1    = Var(self.m.t1, bounds=(self.m.mass_min / self.m.mass_scale, self.m.mass_max / self.m.mass_scale))
        self.m.massdot_1 = Var(self.m.t1, bounds=(self.m.massdot_max/ self.m.mass_scale / self.m.massdot_scale, -0.65 / self.m.mass_scale / self.m.massdot_scale))
        self.m.dmass_dtau_1 = DerivativeVar(self.m.mass_1, wrt=self.m.t1)
        ...
        # mass properties
        self.m.mass_2    = Var(self.m.t2, bounds=(self.m.mass_min / self.m.mass_scale, self.m.mass_max / self.m.mass_scale))
        self.m.massdot_2 = Var(self.m.t2, bounds=(self.m.massdot_max / self.m.mass_scale / self.m.massdot_scale, self.m.massdot_min / self.m.mass_scale / self.m.massdot_scale))
        self.m.dmass_dtau_2 = DerivativeVar(self.m.mass_2, wrt=self.m.t2)
        ...
        # mass properties state variables
        self.m.mass_3    = Var(self.m.t3, bounds=(self.m.mass_min / self.m.mass_scale, self.m.mass_max / self.m.mass_scale))
        self.m.massdot_3 = Var(self.m.t3, bounds=(self.m.massdot_max / self.m.mass_scale / self.m.massdot_scale, -0.05 / self.m.mass_scale / self.m.massdot_scale))
        self.m.dmass_dtau_3 = DerivativeVar(self.m.mass_3, wrt=self.m.t3)
        ...
        # discretizer.apply_to(self.m, wrt=self.m.t1, nfe=60, ncp=3)
        # discretizer.reduce_collocation_points(self.m, var=self.m.mpdot_1, ncp=1, contset=self.m.t1)
        # discretizer.reduce_collocation_points(self.m, var=self.m.eps_1, ncp=2, contset=self.m.t1)
        # discretizer.reduce_collocation_points(self.m, var=self.m.kap_1, ncp=2, contset=self.m.t1)

        # discretizer = TransformationFactory('dae.collocation')
        # discretizer.apply_to(self.m, wrt=self.m.t2, nfe=60, ncp=3)
        ...
        # discretizer = TransformationFactory('dae.collocation')
        # discretizer.apply_to(self.m, wrt=self.m.t3, nfe=60, ncp=3)
